const test = () => {
  ...
  function onClickAnswer(contentAnswers, answer) {
    $(contentAnswers).on('click', answer, function() {
      if (!isAnswer) {
  ...
  function showAnswer() {
    ...
    if (dataAnswer === currentAnswer) {
      $(_this).removeClass('is-active').addClass(correctClass);
      ...
      $(_this).removeClass('is-active').addClass(wrongClass);
    }
    $(btnNext).removeClass('btn-answer');
  ...
  function onClickNextBtn(btnSelector) {
    $(btnSelector).on('click', function(e) {
      e.preventDefault();
      if(!$(btnSelector).hasClass('btn-answer')) {
        nextQuestion();
      } else {
        showAnswer();
      }
